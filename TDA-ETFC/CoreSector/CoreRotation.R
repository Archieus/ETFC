...
TRtxt <- read.csv('coresectortda', header = FALSE, sep = ",")

TRzoo <- read.zoo(TRtxt, sep = ",", format = "%m/%d/%Y", split = 3)
TRxts <- as.xts(TRzoo)
TRxts <- TRxts['2014-11-07:/']

ETFSector <- read.csv("CoreETFNames.csv", header = FALSE)
SectName <- as.data.frame(ETFSector[,2:3])
rownames(SectName) <- ETFSector[,1]
colnames(SectName) <- c("Class", "Sector")
...
names(Signal) <- c( "Class", "Sector","Close","Rank")

write.csv(as.data.frame(ExpReg.rk), file = "CoreRank.csv", row.names = TRUE)
write.csv(Signal, file = "CoreSignal.csv", row.names = TRUE)
